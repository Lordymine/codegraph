...
function oracleStructPrompt(q) {
  ...
  if (q.type === 'callees') {
    return `${common}\nTask: open the body of \`${q.symbol}\` at ${q.file}:${q.line} and list EVERY function/method/hook/component it invokes DIRECTLY (exclude things merely imported but not called, type annotations, and nested-callback calls that belong to a different function). Return items = de-duplicated list of callee names; notes = how you verified.`
  }
